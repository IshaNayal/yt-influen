"""Scrape top 100 Kyra AI influencer transcripts."""
import os
import json
import tempfile
import glob
...
OUTPUT_DIR = r"c:\Users\isha and gaurav\OneDrive\Desktop\yt-influe\Youtube-engagement-analysis-with-scraper\output\ai_influencers\Kyra"
INFLUENCER = "Kyra"
TARGET = 100
...
def get_highest_count():
    filepath = os.path.join(OUTPUT_DIR, "kyra-transcripts.jsonl")
    max_count = 0
...
def main():
    ...
    print(f"\n{'='*70}\nKyra - Virtual Influencer - Top {TARGET} Transcripts\n{'='*70}\nCurrent: {highest}, Target: {TARGET}\nNeed: {max(0, TARGET - highest)} more\n")
    model = whisper.load_model("tiny")
    existing_ids = get_existing_video_ids()
    print(f"Already have: {len(existing_ids)} videos\n")
    all_videos_dict = {}
    queries = ["Kyra virtual influencer interview", "Kyra digital avatar fashion", "Kyra lifestyle content", "Kyra social media posts", "Kyra trending videos", "Kyra makeup tutorial", "Kyra fashion tips", "Kyra behind the scenes", "Kyra brand collaboration", "Kyra Instagram stories", "Kyra vlog", "Kyra photoshoot", "Kyra content creator", "Kyra virtual model"]
    for q in queries:
