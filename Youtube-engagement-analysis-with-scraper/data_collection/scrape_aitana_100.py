"""Scrape top 100 Aitana Lopez AI influencer transcripts."""
import os
import json
import tempfile
import glob
...
OUTPUT_DIR = r"c:\Users\isha and gaurav\OneDrive\Desktop\yt-influe\Youtube-engagement-analysis-with-scraper\output\ai_influencers\Aitana Lopez"
INFLUENCER = "Aitana Lopez"
TARGET = 100

os.makedirs(OUTPUT_DIR, exist_ok=True)

def get_highest_count():
    filepath = os.path.join(OUTPUT_DIR, "aitana-lopez-transcripts.jsonl")
    max_count = 0
...
def main():
    ...
    print(f"\n{'='*70}\nAitana Lopez - Spanish Virtual Influencer - Top {TARGET} Transcripts\n{'='*70}\nCurrent: {highest}, Target: {TARGET}\nNeed: {max(0, TARGET - highest)} more\n")
    model = whisper.load_model("tiny")
    existing_ids = get_existing_video_ids()
    print(f"Already have: {len(existing_ids)} videos\n")
    all_videos_dict = {}
    queries = ["Aitana Lopez first Spanish virtual influencer", "Aitana Lopez interview digital avatar", "Aitana Lopez fashion", "Aitana Lopez lifestyle vlog", "Aitana Lopez Instagram stories", "Aitana Lopez brand deals", "Aitana Lopez Barcelona Spain", "Aitana Lopez social media influencer", "Aitana Lopez virtual model", "Aitana Lopez content creator", "Aitana Lopez makeup fashion", "Aitana Lopez behind scenes", "Aitana Lopez trending", "Aitana Lopez collaboration"]
    for q in queries:
