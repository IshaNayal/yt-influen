"""Add 35 more transcripts for Lu do Magalu with proper counting."""
...
STARTING_COUNT = 73  # Current count
TARGET_NEW_TRANSCRIPTS = 35
FINAL_TARGET = STARTING_COUNT + TARGET_NEW_TRANSCRIPTS

os.makedirs(OUTPUT_DIR, exist_ok=True)

def get_next_count():
    """Get the next count number for the transcript file."""
    filepath = os.path.join(OUTPUT_DIR, "lu-do-magalu-transcripts.jsonl")
    max_count = STARTING_COUNT
    ...
    if os.path.exists(filepath):
        with open(filepath, 'r', encoding='utf-8') as f:
            for line in f:
                if line.strip():
                    try:
                        # Extract count from start of line if it exists
                        parts = line.split(' ', 1)
                        if parts[0].isdigit():
                            count = int(parts[0])
                            if count > max_count:
...
def save_transcript(count, data):
    ...
    with open(filepath, 'a', encoding='utf-8') as f:
        # Write count followed by space, then JSON
        f.write(f"{count} {json.dumps(data, ensure_ascii=False)}\n")

def main():
    print("\n" + "="*70)
    print(f"Lu do Magalu - Adding {TARGET_NEW_TRANSCRIPTS} More Transcripts")
    print("="*70)
    
    print(f"\nCurrent count: {STARTING_COUNT}")
    print(f"Target final count: {FINAL_TARGET}")
    ...
    videos = search_videos("Lu do Magalu", max_results=80)
    ...
    print(f"Starting count number: {next_count}")
    ...
    for i, video in enumerate(new_videos, 1):
        ...
        print(f"\n[{collected + 1}/{TARGET_NEW_TRANSCRIPTS}] #{count_num} {title}...")
