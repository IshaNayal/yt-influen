"""Scrape top 100 Imma AI influencer transcripts."""
import os
import json
import tempfile
import glob
...
OUTPUT_DIR = r"c:\Users\isha and gaurav\OneDrive\Desktop\yt-influe\Youtube-engagement-analysis-with-scraper\output\ai_influencers\Imma"
INFLUENCER = "Imma"
TARGET = 100
...
def get_highest_count():
    filepath = os.path.join(OUTPUT_DIR, "imma-transcripts.jsonl")
    max_count = 0
...
def main():
    ...
    print(f"\n{'='*70}\nImma - Japanese Virtual Influencer - Top {TARGET} Transcripts\n{'='*70}\nCurrent: {highest}, Target: {TARGET}\nNeed: {max(0, TARGET - highest)} more\n")
    model = whisper.load_model("tiny")
    existing_ids = get_existing_video_ids()
    print(f"Already have: {len(existing_ids)} videos\n")
    all_videos_dict = {}
    queries = ["Imma first Japanese virtual influencer", "Imma Japan digital avatar interview", "Imma fashion magazine", "Imma lifestyle vlog", "Imma makeup tutorial", "Imma Instagram content", "Imma Tokyo fashion", "Imma social media star", "Imma brand partnership", "Imma virtual model", "Imma content creator Japan", "Imma trending viral", "Imma photoshoot", "Imma collaboration"]
    for q in queries:
