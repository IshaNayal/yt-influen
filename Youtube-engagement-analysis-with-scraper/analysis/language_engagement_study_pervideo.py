...
def extract_pervideo_bigram_features(df, top_n=100):
    """
    Extract per-video bigram statistics as features.
    For each video, gets its top N bigrams and computes distribution statistics.
    ...
    Returns:
        DataFrame with added bigram feature columns
    """
    from collections import Counter
    
    print("=" * 60)
    print("4. EXTRACTING PER-VIDEO BIGRAM FEATURES")
    print("=" * 60)
    ...
    for text in df['cleaned_text']:
        ...
        probs = [count / total_count for count in bigram_counts.values()]
        entropy = -sum(p * np.log(p) for p in probs if p > 0)
        max_entropy = np.log(total_unique) if total_unique > 1 else 1
        diversity = entropy / max_entropy if max_entropy > 0 else 0
        
        bigram_stats.append({
            'total_unique_bigrams': total_unique,
            'avg_bigram_freq': avg_freq,
            'max_bigram_freq': max_freq,
            'bigram_diversity': diversity
        })
    
    bigram_df = pd.DataFrame(bigram_stats)
    
    print(f"\nPer-video bigram feature statistics:")
    print(bigram_df.describe())
    print()
...
def prepare_features(df, bigram_features_df):
    ...
    # Bigram features
    bigram_features = bigram_features_df.values
    ...
    if 'channel_id' in df.columns:
        ...
        # Combine controls
        X_controls = np.hstack([numeric_controls, channel_encoded])
    else:
    ...
    X_full = np.hstack([X_controls, bigram_features])
...
def train_and_evaluate_models(X_controls, X_full, y, target_name):    # Numeric control variables
    numeric_controls = df[['log_views', 'video_age_days', 'video_duration_seconds']].values
    print(f"Numeric controls shape: {numeric_controls.shape}")
...
def main():
    """Main analysis pipeline with per-video bigrams."""
    print("\n" + "=" * 60)
    print("YOUTUBE LANGUAGE vs ENGAGEMENT: PER-VIDEO BIGRAMS")
    print("=" * 60 + "\n")
    ...
    # 4. Extract per-video bigram features
    bigram_features_df = extract_pervideo_bigram_features(df, top_n=100)
    ...
    X_controls, X_full, encoder = prepare_features(df, bigram_features_df)
    
    # 8. Train and evaluate models
    print("\n" + "=" * 60)
    print("8. TRAINING AND EVALUATING MODELS")
    print("=" * 60)
    ...
    # 9. Interpret coefficients (just show bigram feature importance)
    print("\n" + "=" * 60)
    print("9. BIGRAM FEATURE IMPORTANCE")
    print("=" * 60)
    
    feature_names = ['total_unique_bigrams', 'avg_bigram_freq', 'max_bigram_freq', 'bigram_diversity']
    n_controls = X_controls.shape[1]
    
    print("\nLike Rate - Bigram Feature Coefficients:")
    for i, feat in enumerate(feature_names):
        coef = result_like['model_full_lasso'].coef_[n_controls + i]
        print(f"  {feat}: {coef:.6f}")
    
    print("\nComment Rate - Bigram Feature Coefficients:")
    for i, feat in enumerate(feature_names):
        coef = result_comment['model_full_lasso'].coef_[n_controls + i]
        print(f"  {feat}: {coef:.6f}")
    ...
    metrics_path = output_path / 'model_performance_pervideo.txt'
    with open(metrics_path, 'w') as f:
        f.write("=" * 60 + "\n")
        f.write("YOUTUBE LANGUAGE-ENGAGEMENT: PER-VIDEO BIGRAMS\n")
        f.write("=" * 60 + "\n\n")
